...
{
    public class classExtrasLst
    {
        #region Variables
        List<classExtras> _lstRecords = new List<classExtras>();
        private Stat _objStat;
        #endregion

        #region Atributos
        public List<classExtras> ExtrasLst
        {
            get
            {
        ...
        {
            LoadData(String.Format("SELECT * FROM  CatExtras"));
        }
            ...
            try
            {
                using (DataTable dt = gd.GetDataTable())
                {
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {
                            classExtras info = new classExtras();
                            info.FillField(dr);
                            info.SetPreLoaded();
