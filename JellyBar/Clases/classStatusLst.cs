...
{
    public class classStatusLst
    {
        #region Variables
        List<classStatus> _lstRecords = new List<classStatus>();
        private Stat _objStat;
        #endregion

        #region Atributos
        public List<classStatus> StatusLst
        {
            get
            {
        ...
        {
            LoadData(String.Format("SELECT * FROM  CatStatus"));
        }
            ...
            try
            {
                using (DataTable dt = gd.GetDataTable())
                {
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {
                            classStatus info = new classStatus();
                            info.FillField(dr);
                            info.SetPreLoaded();
