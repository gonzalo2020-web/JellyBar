...
{
    public class classProveedoresLst
    {
        #region Variables
        List<classProveedores> _lstRecords = new List<classProveedores>();
        private Stat _objStat;
        #endregion

        #region Atributos
        public List<classProveedores> ProveedoresLst
        {
            get
            {
        ...
        {
            LoadData(String.Format("SELECT * FROM  CatProveedores"));
        }
            ...
            try
            {
                using (DataTable dt = gd.GetDataTable())
                {
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {
                            classProveedores info = new classProveedores();
                            info.FillField(dr);
                            info.SetPreLoaded();
